...
def test_default_config_selects_power_visible_profile() -> None:
    ...
    assert resolve_intrusion_weights(config) == "models/delivery/intrusion_power_visible_yolo11l.pt"
...
def test_default_breaker_uses_mobile_device_detection() -> None:
    ...
    assert config.models.breaker == "models/delivery/breaker_mobile_types_yolo11s.pt"
    assert config.models.breaker_state_classifier == "models/delivery/breaker_mcb_state_yolo11s_cls.pt"
    assert config.breaker.mobile_device_classes == ["MCB"]
    assert config.breaker.mobile_class_limits == {"MCB": 10}
