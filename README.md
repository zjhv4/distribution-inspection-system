...
断路器模块面向移动巡检画面，先实时识别 `MCB`、`RCD` 和 `ISOLATOR`，再只对检测到的 MCB 读取拨杆位置。设备不在画面中时不显示标签；拨杆被遮挡或看不清时输出 `UNKNOWN`。`TRIP` 与 `MICRO_TRIP` 根据同一设备的连续状态生成，不作为单帧图像类别。
...
- 断路器设备模型测试集 Precision 为 0.980，Recall 为 0.977，mAP50 为 0.984；
